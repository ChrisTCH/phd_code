...
sync_fits = fits.open(data_loc + 'b.1p2_synint_p1-4.fits')
...
for i in range(sync_shape[0]):
	...
	sync_sq_mean = np.power( np.mean(sync_data[i]), 2.0 )

	# Calculate the mean of the synchrotron intensity values squared
	sync_mean_sq = np.mean( np.power(sync_data[i], 2.0) )
...
plt.savefig(data_loc + 'Sync_Int_Corr_Comp_6.png', format = 'png')
...
# Save the figure using the given filename and format
plt.savefig(data_loc + 'Sync_Int_Max_Diff_6.png', format = 'png')
